...
// TestHostedLabelsAreReserved documents the platform rule Homerun is built
...
func TestHostedLabelsAreReserved(t *testing.T) {
	for _, l := range []string{"ubuntu-latest", "UBUNTU-LATEST", "macos-latest", "windows-latest", "ubuntu-22.04"} {
		if !IsReserved(l) {
			t.Errorf("IsReserved(%q) = false; Homerun would wrongly promise interception", l)
		}
	}
	for _, l := range []string{"homerun", "self-hosted", "linux", "my-custom-label"} {
		if IsReserved(l) {
...
func TestDefaultLabelsNeverClaimHostedNames(t *testing.T) {
	...
	if !found {
		t.Errorf("default labels must include %q so `homerun adopt` output routes here", Homerun)
	}
}
...
func TestMatchesIsSupersetSemantics(t *testing.T) {
	runner := []string{"self-hosted", "linux", "x64", "homerun", "homerun-linux"}
	...
	cases := []struct {
		...
		{[]string{"homerun"}, true, "homerun label"},
...
// TestAdoptRewritesHostedLabels covers the core of `homerun adopt`.
...
`
	...
	if !strings.Contains(res.Content, "runs-on: [self-hosted, homerun, homerun-linux]") {
		t.Errorf("linux job not rewritten correctly:\n%s", res.Content)
	}
	if !strings.Contains(res.Content, "runs-on: [self-hosted, homerun, homerun-macos]") {
...
func TestAdviseIsHonestAboutHostedLabels(t *testing.T) {
	...
	if !strings.Contains(adv.NextStep, "adopt") {
		t.Errorf("advice should point at `homerun adopt`, got %q", adv.NextStep)
	}

	adv = Advise([]string{Homerun}, runnerLabels)
	if !adv.Interceptable {
		t.Error("a workflow already targeting the homerun label should route today")
	}
}

// TestProbeDetectsDroppedLabels backs the "verify reality" strategy: if GitHub
// ever accepts a hosted label, Homerun notices without a code change.
func TestProbeDetectsDroppedLabels(t *testing.T) {
	res := Probe([]string{"homerun", "ubuntu-latest"}, []string{"self-hosted", "homerun"})
	...
	res = Probe([]string{"homerun", "ubuntu-latest"}, []string{"homerun", "ubuntu-latest"})
	if !res.HostedLabelAccepted {
