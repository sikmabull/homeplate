...
// StatusContextPrefix namespaces every commit status Homerun creates so it is
// visually distinct from GitHub's own "ci/..." contexts in the PR checks list.
const StatusContextPrefix = "homerun"
...
func Disclaimer(at time.Time) string {
	return fmt.Sprintf("ran locally via Homerun offline mode at %s", at.UTC().Format(time.RFC3339))
}

// StatusContext builds the context string for a job, e.g.
// "homerun/ci.yml / build (offline)".
...
func (r *Replayer) replayJob(ctx context.Context, j *store.Job, out *Outcome) error {
	...
	if attempts >= MaxAttempts {
		r.logf("job %d exceeded %d replay attempts; giving up (see `homerun logs %d`)", j.ID, MaxAttempts, j.ID)
		out.Skipped++
		return r.DB.MarkJobReplayed(ctx, j.ID)
	}
...
func (r *Replayer) autoApproveAndMerge(ctx context.Context, j *store.Job, out *Outcome, client *ghapi.Client) error {
	...
	for _, pr := range prs {
		...
		// Every other Homerun job on this commit must also have passed.
		...
		if !done {
			body := fmt.Sprintf("Approved by Homerun: all required checks %s.\n\n"+
		...
		res, err := client.MergePR(ctx, j.RepoSlug, pr.Number, ghapi.MergeRequest{
			...
			CommitMessage: fmt.Sprintf("Merged by Homerun after a local run.\n%s",
				Disclaimer(r.now())),
		})
		if err != nil {
...
// allLocalChecksPassed verifies every Homerun job recorded for a commit passed.
