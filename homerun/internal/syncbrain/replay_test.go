...
func TestReplayNeverImpersonatesGitHub(t *testing.T) {
	...
	if !strings.Contains(desc, "ran locally via Homerun offline mode at") {
		t.Errorf("status description must disclose local execution, got %q", desc)
	}
	context_, _ := gh.statuses[0]["context"].(string)
	if !strings.HasPrefix(context_, "homerun") {
		t.Errorf("status context must be namespaced to homerun, got %q", context_)
	}
