...
func annotateAdminErr(err error, target, kind string) error {
	switch {
	case IsNotFound(err):
		return fmt.Errorf("%w\n  -> Homerun could not mint a runner registration token for %s %s.\n"+
...
// Homerun does not post a duplicate.
...
// CheckRun is the Checks API payload. Homerun attempts this first when the
...
// PullRequest is the subset of PR fields Homerun uses.
...
// MergePR merges a pull request. SHA is required by Homerun so that a merge
