...
CONFIG_ARGS=(
  ...
  --token "${TOKEN}"
  --name "${HOMERUN_NAME}"
  --labels "${HOMERUN_LABELS}"
  --work /home/runner/_work
)

if [ -n "${HOMERUN_GROUP:-}" ]; then
  CONFIG_ARGS+=(--runnergroup "${HOMERUN_GROUP}")
fi

# Never echo the token, even under set -x.
./config.sh "${CONFIG_ARGS[@]}" 2>&1 | sed -e "s/${TOKEN}/***/g"

echo "homerun: runner configured as ephemeral; awaiting exactly one job"
...
RUN useradd -m -u 1001 -s /bin/bash runner \
    ...
    && mkdir -p /home/runner/actions-runner /home/runner/_work /run/homerun \
...
// ImageBuilder assembles the Homerun runner image.
...
func ImageTag(runnerVersion, baseImage string) string {
	...
	return fmt.Sprintf("homerun/runner:%s-%s", runnerVersion, hex.EncodeToString(sum[:])[:8])
}
...
func (b *ImageBuilder) Ensure(ctx context.Context, runnerVersion string, log io.Writer) (string, error) {
	...
	if _, err := os.Stat(tarballDest); err != nil {
		...
		if _, err := os.Stat(src); err != nil {
			fmt.Fprintf(log, "homerun: downloading actions/runner %s for %s\n", runnerVersion, plat)
	...
	fmt.Fprintf(log, "homerun: building clean-room image %s (one time, ~2 min)\n", tag)
