// Package config holds Homerun's on-disk configuration (~/.homerun/config.toml).
//
...
// Config is the root of ~/.homerun/config.toml.
type Config struct {
...
// Power controls when Homerun is allowed to pick up work and when it holds
// a sleep assertion.
type Power struct {
	...
	// RunOnBattery permits picking up jobs while unplugged at all.
	...
	// AllowClamshellPmset permits `sudo pmset -b disablesleep 1`. Off by
	// default: it is a global system change and requires root.
...
// Dir returns the Homerun home directory, honouring $HOMERUN_HOME for tests.
func Dir() string {
	if v := os.Getenv("HOMERUN_HOME"); v != "" {
		return v
	}
	...
	return filepath.Join(home, ".homerun")
}
...
// is not an error: defaults are returned so `homerun status` works pre-init.
