...
// must be opted into with HOMERUN_KEYRING=file. It is never selected
...
// Service is the keychain service name under which Homerun stores tokens.
const Service = "homerun-ci"
...
type Store interface {
	...
	// Name identifies the backend for `homerun doctor`.
	Name() string
}
...
func Open() (Store, error) {
	...
	switch strings.ToLower(os.Getenv("HOMERUN_KEYRING")) {
	case "file":
		return newFileStore()
	case "keychain":
		return &macStore{}, nil
	...
	case "linux":
		if _, err := exec.LookPath("secret-tool"); err != nil {
			return nil, fmt.Errorf("keyring: secret-tool not found. Install libsecret-tools " +
				"(apt install libsecret-tools / dnf install libsecret) or set HOMERUN_KEYRING=file " +
				"to accept plaintext token storage")
		}
		return &secretToolStore{}, nil
	default:
		return nil, fmt.Errorf("keyring: no OS credential store for %s; set HOMERUN_KEYRING=file to accept plaintext", runtime.GOOS)
	}
}
...
func (m *macStore) Set(key, value string) error {
	...
	cmd := exec.Command("security", "add-generic-password",
		...
		"-D", "Homerun GitHub token",
		"-j", "Managed by Homerun. Delete with: homerun auth remove <profile>")
...
func (s *secretToolStore) Set(key, value string) error {
	cmd := exec.Command("secret-tool", "store", "--label=Homerun GitHub token",
...
func newFileStore() (Store, error) {
	dir := os.Getenv("HOMERUN_KEYRING_DIR")
	if dir == "" {
		home := os.Getenv("HOMERUN_HOME")
		if home == "" {
			h, err := os.UserHomeDir()
			if err != nil {
				return nil, err
			}
			home = filepath.Join(h, ".homerun")
		}
