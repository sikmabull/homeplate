...
// TestPlaintextIsNeverAutomatic is a security guarantee: Homerun must never
// silently downgrade to plaintext token storage.
func TestPlaintextIsNeverAutomatic(t *testing.T) {
	t.Setenv("HOMERUN_KEYRING", "")
	SetOverride(nil)
...
func TestRealOSKeychainRoundTrip(t *testing.T) {
	...
	t.Setenv("HOMERUN_KEYRING", "keychain")
	SetOverride(nil)
	t.Cleanup(func() { SetOverride(nil) })
	...
	const key = "homerun-selftest@github.com"
