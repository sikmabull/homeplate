...
func newLimitCmd() *cobra.Command {
  ...
  homerun limit                            show current caps
  homerun limit --cpus 4 --memory 8g       change them (takes effect immediately)
...
func runDoctor(ctx context.Context) error {
	...
	if ring, err := keyring.Open(); err != nil {
		...
		results = append(results, checkResult{
			...
			Fix: map[bool]string{true: "unset HOMERUN_KEYRING to use the OS keychain"}[warn],
		})
	}
	...
	if aerr != nil {
		...
		if len(profiles) == 0 {
			results = append(results, checkResult{
				Name: "identities", Detail: "none configured", Fix: "homerun auth add personal",
			})
		} else {
			var names []string
			bad := 0
			...
			results = append(results, checkResult{
				...
				Fix: map[bool]string{true: "re-run `homerun auth add <profile>`"}[bad > 0],
			})
		}
	}
	...
	results = append(results, checkResult{
		...
		Detail: cs.Reason,
		Fix:    map[bool]string{true: "Homerun will run jobs offline via act and replay results when Actions recovers"}[cs.UseOffline()],
	})
	...
	if inst, err := service.New(config.Dir()); err == nil {
		...
		results = append(results, checkResult{
			...
			Fix: map[bool]string{true: "homerun service install"}[!installed],
		})
	}
	...
	switch {
	case fails > 0:
		fmt.Printf("%d problem(s), %d warning(s). Homerun will not work correctly until the problems are fixed.\n\n", fails, warns)
	case warns > 0:
...
func routingCheck(ctx context.Context, cfg *config.Config) []checkResult {
	...
	if len(cfg.Repos) == 0 {
		return []checkResult{{
			Name: "label routing", Warn: true, Detail: "no repos linked yet", Fix: "homerun link",
		}}
	}
