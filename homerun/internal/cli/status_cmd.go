...
import (
	...
	"github.com/homerun-ci/homerun/internal/savings"
...
func runStatus(ctx context.Context, explain bool) error {
	...
	if err == nil {
		...
		if len(profiles) == 0 {
			fmt.Println("IDENTITIES   none (run `homerun auth add personal`)")
		} else {
	...
	if _, err := os.Stat(daemon.PauseFile(config.Dir())); err == nil {
		fmt.Println("  PAUSED by `homerun pause` - run `homerun resume` to accept jobs again")
	}
	...
	if explain {
		...
		fmt.Println()
		fmt.Printf("  (run `homerun status --explain-savings` to audit the $%.2f figure)\n", sum.USD)
	}
	fmt.Println()
	return nil
}
...
func newPauseCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			fmt.Println("Resume with: homerun resume")
			return nil
		},
	}
}
