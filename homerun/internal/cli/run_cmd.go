...
Engine B is BETA - act is not byte-identical to GitHub's runner. See README.`,
		RunE: func(c *cobra.Command, args []string) error {
			...
			if !created {
				...
				if j.State.Terminal() {
					fmt.Println("Re-running the same commit+workflow is deduplicated. Use `homerun logs`.")
					return nil
				}
			}
			...
			if claimed == nil || claimed.ID != j.ID {
				fmt.Println("The daemon claimed this job first; follow it with `homerun logs`.")
				return nil
			}
			...
			opts := offline.RunOpts{
				...
				Event: offline.Event{
					...
					Repository: slug, Actor: "homerun", Branch: branch,
				},
			...
			// `homerun logs` and the replayed status context are meaningful.
			...
			fmt.Println("on the next sync, clearly labelled as having run locally via Homerun.")
			return nil
		},
	}
...
// inferSlug derives owner/repo from the origin remote.
func inferSlug(ctx context.Context, dir string) (string, error) {
	...
	return "", fmt.Errorf("origin %q is not a recognisable GitHub URL", url)
}
