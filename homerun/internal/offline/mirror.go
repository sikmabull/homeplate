...
// in Homerun's README. Homerun shells out to the `act` binary rather than
...
//
// Engine B must run workflows against REAL commits while offline, so Homerun
...
//
// remoteURL should be an authenticated HTTPS URL or an SSH URL. Homerun passes
// x-access-token style HTTPS so that no SSH key is required, and deliberately
// never writes the token into the repo config (it is passed per-invocation).
func EnsureMirror(ctx context.Context, homeDir, slug, remoteURL string, log func(string)) (*Mirror, error) {
	...
	if _, err := os.Stat(filepath.Join(path, "HEAD")); err == nil {
		...
		cmd := exec.CommandContext(ctx, "git", "--git-dir", path, "fetch", "--prune", "--tags", remoteURL,
			"+refs/heads/*:refs/heads/*", "+refs/pull/*/head:refs/pull/*/head")
		if out, err := cmd.CombinedOutput(); err != nil {
			// git may echo the remote URL (which carries the token) in its
			// error text, so scrub the captured output before it reaches a log.
	...
	cmd := exec.CommandContext(ctx, "git", "clone", "--mirror", remoteURL, path)
	if out, err := cmd.CombinedOutput(); err != nil {
...
// sometimes echo that URL back, which would write a live token into Homerun's
...
// MirrorInfo reports mirror freshness for `homerun status`.
