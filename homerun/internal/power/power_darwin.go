...
// cgo + IOPMAssertionCreateWithName so that Homerun stays a pure-Go,
...
func clamshell(ctx context.Context, s State, holdEnabled bool) ClamshellVerdict {
	...
	if !s.OnAC() {
		return ClamshellVerdict{
			...
			Reason: "this is a MacBook on battery power. Closing the lid on battery sleeps " +
				"the machine regardless of any sleep assertion Homerun can hold",
	...
	return ClamshellVerdict{
		...
		Remedy: "attach an external display (Apple's documented clamshell requirement: AC + " +
			"external display + external keyboard/mouse), OR keep the lid open, OR enable " +
			"allow_clamshell_pmset = true in config.toml to permit `sudo pmset disablesleep 1` " +
...
func platformNotes() []string {
	...
	if os.Geteuid() != 0 {
		notes = append(notes,
			"macOS: `sudo pmset disablesleep 1` does block lid-close sleep (it sets the "+
				"system-wide SleepDisabled bit), but it is undocumented, requires root, and "+
				"applies to the whole system. Homerun never sets it unless "+
				"allow_clamshell_pmset = true.")
	}
	return notes
}
