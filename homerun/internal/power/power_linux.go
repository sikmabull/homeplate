...
// lid-close suspend - no private entitlement, no root. That is why Homerun's
...
// InhibitWhat is the event set Homerun blocks while jobs are running.
...
func acquireAssertion(reason string) (assertion, error) {
	...
	if reason == "" {
		reason = "Homerun is running CI jobs"
	}
...
func clamshell(ctx context.Context, s State, holdEnabled bool) ClamshellVerdict {
	...
	if !holdEnabled {
		return ClamshellVerdict{
			...
			Remedy:          "homerun limit --hold-sleep true",
		}
	}
	// logind honours a `handle-lid-switch` inhibitor lock, which Homerun takes
	...
	if !s.OnAC() {
		return ClamshellVerdict{
			WillKeepRunning: true,
			Reason: "Homerun holds a systemd-inhibit lock including handle-lid-switch while jobs " +
...
func logindLidSetting() string {
	for _, p := range []string{"/etc/systemd/logind.conf", "/etc/systemd/logind.conf.d/homerun.conf"} {
...
func platformNotes() []string {
	return []string{
		...
		"Linux: an inhibitor lock only applies while Homerun holds it, i.e. while work exists. " +
