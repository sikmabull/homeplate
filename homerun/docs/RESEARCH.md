# Homerun — Research Dossier (facts, endpoints, verdicts)
...
observable behaviour regressed in mid-2025 with no acknowledgement. Homerun must not depend on it.
...
what Homerun wants. **UNVERIFIED:** whether, with standard hosted runners disabled, a self-hosted
...
### 1.6 Practical implication for Homerun
...
1. Edit workflows (one-line `runs-on:` change), possibly via a Homerun-opened PR.
...
Canonical ephemeral invocation for Homerun:
```bash
./config.sh --unattended --replace --ephemeral --disableupdate \
  --url https://github.com/OWNER/REPO \
  --token "$REG_TOKEN" \
  --name "homerun-$(hostname)-$(uuidgen)" \
  --labels "homerun,macos,arm64" \
  --work _work
...
**Interpretation (important for Homerun):**
...
* Binaries are **ad-hoc signed and not notarized**, so `spctl` rejects them. This matters **only if a
  ...
  Safari/Chrome/Mail do. If Homerun downloads the tarball itself over HTTP, Gatekeeper will not
  block it.
...
* Known live bug to be aware of: multiple open issues report **v2.336.0 deadlocks on macOS arm64**
  ...
  <https://github.com/actions/runner/pull/4572>. **Homerun should pin/prefer v2.335.1 on macOS arm64
...
**Recommendation:** register a first-party "Homerun" OAuth App (or GitHub App) and enable device
...
| Edit workflow files | `workflow` (in addition to `repo`) | Workflows: write | needed if Homerun ever commits `.github/workflows/*` |

Minimal practical Homerun scope set (classic/device flow): **`repo read:org workflow`**, plus
...
### 5.7 Recommended counter formula for Homerun
```
...
```json
{
  ...
  "target_url": "https://…",                  // deep link (Homerun: local log viewer / gist)
  "description": "short text",
  "context": "homerun/build"                  // default "default"; case-insensitive
}
```
Commit statuses show in the PR "checks" box and can be required by branch protection, so this is a
fully workable replay channel for Homerun without becoming a GitHub App.
...
* **Status API:** **no** log/artifact attachment at all. Only `target_url` + a short `description`.
  Homerun must host the log itself (local HTTP viewer, a gist via the `gist` scope, a release asset,
...
a good argument for Homerun surfacing this endpoint in its UI.)
...
```
i.e. the image value `-self-hosted` means "run on this host". **This is directly relevant to Homerun**:
...
```
**There is no `internal/` directory in the repo** — every package Homerun needs is importable:
...
* Requires **root**. It persists across reboots until set back to 0 — a Homerun that sets it MUST
...
* Side effects: **all** sleep is disabled, including low-power/thermal-motivated demand sleep paths
  ...
  thermal risk; Homerun should hold it only for the duration of a job and warn the user.
...
**Recommended Homerun policy on macOS:**
...
| macOS | Linux |
|---|---|
| `caffeinate -i <cmd>` | `systemd-inhibit --what=idle:sleep --who=homerun --why="CI job running" <cmd>` |
