// Package service installs Homeplate's daemon as a user-level background service.
//
...
const Label = "com.homeplate.daemon"

// UnitName is the systemd user unit filename.
const UnitName = "homeplate.service"

// Installer manages the platform service.
type Installer struct {
	// BinPath is the absolute path to the homeplate binary.
	BinPath string
	// HomeDir is ~/.homeplate.
	HomeDir string
}
...
<dict>
    ...
    <!-- Homeplate jobs are background work; this keeps the Mac responsive.
...
Description=Homeplate - local GitHub Actions runner
Documentation=https://github.com/homeplate-ci/homeplate
...
// but cannot find `docker` or `act`, which looks like a Homeplate bug.
...
func (i *Installer) Status() (installed bool, running bool, detail string) {
	...
	if _, err := os.Stat(path); err != nil {
		return false, false, "not installed (run `homeplate init` or `homeplate service install`)"
	}
	switch runtime.GOOS {
	case "darwin":
