...
func TestRenderProducesValidUnit(t *testing.T) {
	i := &Installer{BinPath: "/opt/homebrew/bin/homeplate", HomeDir: "/Users/dev/.homeplate"}
	out, err := i.Render()
	if err != nil {
		t.Fatalf("Render: %v", err)
	}

	if !strings.Contains(out, "/opt/homebrew/bin/homeplate") {
		t.Error("unit does not reference the homeplate binary")
	}
	...
	case "darwin":
		for _, must := range []string{
			"<?xml", "com.homeplate.daemon", "<key>RunAtLoad</key>", "<key>KeepAlive</key>",
			"<key>ThrottleInterval</key>", "Background", "HOMEPLATE_HOME",
		} {
	...
	case "linux":
		for _, must := range []string{
			...
			"WantedBy=default.target", "HOMEPLATE_HOME=",
		} {
			if !strings.Contains(out, must) {
...
func TestRenderIsDeterministic(t *testing.T) {
	i := &Installer{BinPath: "/usr/local/bin/homeplate", HomeDir: "/home/dev/.homeplate"}
	first, err := i.Render()
	if err != nil {
		t.Fatal(err)
	}
...
func TestPathLocation(t *testing.T) {
	i := &Installer{BinPath: "/x/homeplate", HomeDir: "/x/.homeplate"}
	p, err := i.Path()
	if err != nil {
