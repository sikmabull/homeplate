...
func newAuthCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `Homeplate supports multiple GitHub identities as named profiles.
...
func warnMissingScopes(scopes []string, kind auth.Kind) {
	...
	if !have["repo"] {
		fmt.Println("\n  ! Missing `repo` scope: Homeplate cannot register repo-level runners or post statuses.")
	}
	if !have["admin:org"] {
		fmt.Println("  ! Missing `admin:org` scope: org-level runner registration will fail (repo-level still works).")
	}
	if !have["workflow"] {
		fmt.Println("  ! Missing `workflow` scope: `homeplate adopt` cannot open PRs that edit workflow files.")
	}
}
...
func newAuthListCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			profiles := st.List()
			if len(profiles) == 0 {
				fmt.Println("No identities configured. Run: homeplate auth add personal")
				return nil
			}
