...
then registers Homeplate as a self-hosted runner for each selection.
...
Actions settings) that pull request's code can execute on YOUR machine.`,
		RunE: func(c *cobra.Command, args []string) error {
			...
			fmt.Printf("\n    homeplate adopt %s\n", selections[0].Slug)
			fmt.Println("\n  That opens one PR rewriting `runs-on:` to Homeplate's labels.")
			return nil
		},
	}
	...
	for _, slug := range slugs {
		if !strings.Contains(slug, "/") {
			...
			out = append(out, config.LinkedRepo{
				...
				Labels: extra, RunnerGroup: group, LinkedAt: time.Now().UTC(),
			})
			continue
		}
...
func linkInteractive(ctx context.Context, client *ghapi.Client, profile *auth.Profile,
	...
	for _, i := range picked {
		...
		lr := config.LinkedRepo{
			...
			Labels: extra, RunnerGroup: group, LinkedAt: time.Now().UTC(),
			LocalPath: findLocalClone(ch.slug),
		}
		if ch.scope == "repo" {
...
func printPublicWarning(slug string) {
  ...
  !! %s is PUBLIC.
     ...
     Homeplate runs jobs in throwaway containers with no host network, but a
