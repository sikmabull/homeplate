...
func newInitCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "init",
		Short: "Set up Homeplate: authenticate, pick repos, install the daemon",
		RunE: func(c *cobra.Command, args []string) error {
			...
			fmt.Println("Homeplate - your machine, your CI, up to 31x cheaper than hosted")
			...
			if d, err := runner.NewDocker(); err != nil {
				...
				fmt.Println("   Homeplate needs Docker (or Podman/Colima) for clean-room Linux jobs.")
				fmt.Println("   Install:  brew install --cask docker")
				fmt.Println("   Then re-run: homeplate init")
			} else if err := d.Available(ctx); err != nil {
				fmt.Println("NOT RUNNING")
				fmt.Printf("   %v\n   Start Docker Desktop, then re-run `homeplate doctor`.\n", err)
			} else {
			...
			if len(cfg.Repos) == 0 {
				...
				fmt.Println("   (run `homeplate link` now, or later)")
			} else {
			...
			if !skipService {
				...
				if err != nil {
					...
					fmt.Printf("FAILED: %v\n", err)
					fmt.Println("   You can run it in the foreground: homeplate daemon run")
				} else {
			...
			fmt.Println("\n" + strings.Repeat("=", 52))
			fmt.Println("Next:")
			fmt.Println("  homeplate link                 pick repos")
			fmt.Println("  homeplate adopt <owner/repo>   route existing workflows here")
			fmt.Println("  homeplate status               see what it's saving you")
			fmt.Println("  homeplate doctor               diagnose anything odd")
			return nil
		},
	}
...
func runDaemon(ctx context.Context) error {
	...
	for _, p := range authStore.List() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "homeplate: profile %q has no usable token: %v\n", p.Name, err)
			continue
		}
		c := ghNew(token)
	...
	if len(clients.Profiles()) == 0 {
		return fmt.Errorf("no usable identities; run `homeplate auth add <name>`")
	}

	d := daemon.New(cfg, db, config.Dir(), clients, os.Stdout)
	d.Idle = power.NewBusyTracker(cfg.Power.IdleUserCPUPercent)
	// Point connectivity probing at the right API for GHES shops.
	if p, err := authStore.Default(); err == nil && p.Host != "" && p.Host != "github.com" {
		d.Conn.APIHost = "api." + p.Host
	}
	if dk, err := runner.NewDocker(); err == nil {
		d.Docker = dk
	}
	// GHES-aware target expansion: a target's host comes from its profile, so
	// registration URLs are built for the right instance.
	hostFor := func(profile string) string {
		if p, err := authStore.Get(profile); err == nil {
			return p.Host
		}
		return ""
	}
	d.SetTargets(daemon.TargetsFromConfig(cfg, hostFor))
