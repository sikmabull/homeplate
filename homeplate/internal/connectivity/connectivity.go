...
//
// Homeplate distinguishes three states, because they demand different responses:
//
...
// UseOffline reports whether the snapshot implies Engine B should handle work.
func (s Status) UseOffline() bool { return s.State.UseOffline() }

// Status is a full connectivity snapshot with reasons.
type Status struct {
	State State
	// Reason is always populated, so the daemon log and `homeplate status`
