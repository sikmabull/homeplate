...
// it is NOT byte-identical to GitHub's runner. Known divergences that Homeplate
...
//     GitHub by Homeplate (see README "Known limits").
...
// MinimumVersion is the oldest act Homeplate is tested against.
...
//
// Homeplate constructs this from local git state so that `github.sha`,
...
func (a *Act) Args(o RunOpts, eventPath string) []string {
	...
	// Map every hosted Linux label AND Homeplate's own labels onto the image, so
	// a repo that has already run `homeplate adopt` still works offline.
	for _, l := range []string{
		"ubuntu-latest", "ubuntu-24.04", "ubuntu-22.04", "ubuntu-20.04",
		labels.Homeplate, "homeplate-linux", "self-hosted",
	} {
		args = append(args, "-P", l+"="+img)
	}

	// Resource caps: act forwards container options verbatim to Docker, which
	// is how Homeplate's max_cpus/max_memory apply to Engine B as well.
	var copts []string
...
var (
	...
	// or ASCII depending on TTY. Homeplate parses the stable ASCII markers.
...
func (a *Act) Run(ctx context.Context, o RunOpts, logPath string, live io.Writer) (*Result, error) {
	...
	eventFile, err := os.CreateTemp("", "homeplate-event-*.json")
	if err != nil {
		return res, err
	}
...
func truncate(s string, n int) string {
	...
	return s[:n] + "\n... (truncated by homeplate; full log on disk)"
}
