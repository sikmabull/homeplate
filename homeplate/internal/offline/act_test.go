...
const realActOutput = "\x1b[33m[CI/build] \x1b[0m\u2b50 Run Set up job\n" +
	...
	"\x1b[33m|\x1b[0m hello from homeplate offline mode\n" +
...
// TestActMapsHostedAndHomeplateLabels: offline mode must run workflows whether
// or not they have been through `homeplate adopt`.
func TestActMapsHostedAndHomeplateLabels(t *testing.T) {
	a := &Act{Bin: "act"}
	joined := strings.Join(a.Args(RunOpts{Event: Event{Name: "push"}, Image: "img:tag"}, "/e.json"), " ")
	for _, label := range []string{"ubuntu-latest=img:tag", "homeplate=img:tag", "self-hosted=img:tag"} {
...
// write a live token straight into Homeplate's logs.
