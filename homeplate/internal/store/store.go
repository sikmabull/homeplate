// Package store is Homeplate's durable state: the job queue, run results, and
...
const (
	// EngineA is the official actions/runner wrapped by Homeplate (connected).
...
func Path(homeDir string) string { return filepath.Join(homeDir, "homeplate.db") }
...
// RecentJobs returns the newest jobs for `homeplate status`/`logs`.
...
// FailReplay records an attempt failure for backoff and `homeplate status`.
...
// Stats summarises the queue for `homeplate status`.
type Stats struct {
	Queued         int
	Running        int
	SucceededToday int
