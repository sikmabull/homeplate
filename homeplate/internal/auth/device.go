...
//
// Homeplate ships with no client_id baked in. The GitHub CLI's client_id is
...
// impersonate another product. Homeplate will not do that by default. Options:
//
//  1. Register your own OAuth App (30 seconds, no secret needed for device
//     flow), then `homeplate auth add work --client-id Iv1.xxxxxxxx`
//     or export HOMEPLATE_GITHUB_CLIENT_ID.
//  2. Distributors set it at build time:
//     -ldflags "-X github.com/homeplate-ci/homeplate/internal/auth.DefaultClientID=Iv1.xxxx"
//  3. Skip OAuth entirely: `homeplate auth add work --pat` (fine-grained PAT).
var DefaultClientID = ""

// Scopes required for full Homeplate operation with a classic OAuth token.
//
...
//	workflow     - required only for `homeplate adopt` (rewriting workflow YAML)
//
...
func NewDeviceFlow(clientID, host string, scopes []string) (*DeviceFlow, error) {
	if clientID == "" {
		clientID = os.Getenv("HOMEPLATE_GITHUB_CLIENT_ID")
	}
	if clientID == "" {
	...
	if clientID == "" {
		return nil, errors.New(
			"device flow needs an OAuth App client_id.\n" +
				"  Fastest path:  homeplate auth add <name> --pat        (paste a fine-grained PAT)\n" +
				...
				"    homeplate auth add <name> --client-id Iv1.xxxxxxxxxxxx\n" +
				"  or export HOMEPLATE_GITHUB_CLIENT_ID=Iv1.xxxxxxxxxxxx")
	}
	if host == "" {
		host = "github.com"
	}
...
func (f *DeviceFlow) postForm(ctx context.Context, endpoint string, form url.Values, out any) error {
	...
	req.Header.Set("User-Agent", "homeplate")
	...
	if resp.StatusCode >= 500 {
		return fmt.Errorf("device flow: GitHub returned %s (GitHub may be degraded; check `homeplate doctor`)", resp.Status)
	}
