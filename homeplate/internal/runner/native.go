...
// macOS jobs have the softer guarantee, and `homeplate status` says so.
...
func (n *NativeEngine) RunOne(ctx context.Context, t Target, log io.Writer) (*JobOutcome, error) {
	...
	template, err := n.Cache.Ensure(ctx, rel, func(m string) { fmt.Fprintln(log, "homeplate:", m) })
	if err != nil {
		return out, err
	}
	...
	labelList := t.Labels
	if len(labelList) == 0 {
		labelList = labels.MacOSDefault()
	}

	// Prefer a JIT config (no reusable secret on disk); fall back to the
	// registration-token flow (older GHES, named runner groups, test fakes).
	jit, jitErr := n.mintJIT(ctx, t, runnerName, labelList)
	useJIT := jitErr == nil && jit != nil && jit.EncodedJITConfig != ""
	var tok *ghapi.RegistrationToken
	if !useJIT {
		if jitErr != nil {
	...
	// --- configure (ephemeral), unless the JIT config already encodes it ---
	if !useJIT {
		cfgArgs := []string{
	...
	runArgs := []string{}
	if useJIT {
		runArgs = append(runArgs, "--jitconfig", jit.EncodedJITConfig)
	}
	var cmd *exec.Cmd
	switch {
	case n.UseTaskPolicy && hasTaskPolicy():
		...
		cmd = exec.CommandContext(ctx, "taskpolicy", append([]string{"-b", runPath}, runArgs...)...)
	case n.NiceLevel > 0:
		cmd = exec.CommandContext(ctx, "nice", append([]string{"-n", strconv.Itoa(n.NiceLevel), runPath}, runArgs...)...)
	default:
...
// mintJIT tries the JIT path; (nil, nil) means "minter cannot JIT".
func (n *NativeEngine) mintJIT(ctx context.Context, t Target, name string, labelList []string) (*ghapi.JITConfig, error) {
	jm, ok := n.Minter.(JitMinter)
	if !ok || t.RunnerGroup != "" {
		return nil, nil
	}
...
func scrubbedEnv() []string {
	...
	out = append(out, "HOMEPLATE=1", "CI=true", "RUNNER_MANUALLY_TRAP_SIG=1")
	return out
}
