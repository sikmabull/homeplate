...
// Target is one linked repo or org that this machine serves, for ONE
// execution class. A Mac typically serves each repo twice: once as a
// linux-class target (Docker container) and once as a macos-class target
// (native engine). Keeping them separate is what stops a Linux container
// from ever claiming a macOS job.
type Target struct {
	...
	RunnerGroup string
	Labels      []string
	// Class selects the execution engine: linux -> Docker, macos -> native.
	Class labels.Class
	// Host is the GitHub hostname ("github.com" or a GHES host). Registration
	// URLs and the runner's --url are derived from it; without it a GHES
	// target would silently register against github.com.
	Host string
}
...
// JitMinter is the optional generate-jitconfig capability. *ghapi.Client
// satisfies it; test fakes and older GHES instances do not have to.
type JitMinter interface {
	GenerateJITConfigFor(ctx context.Context, scope, slug string, req ghapi.JITConfigRequest) (*ghapi.JITConfig, error)
}

// mintJIT tries the JIT path. Returns (nil, nil) when the minter cannot do
// JIT at all (not an error - just use the token flow).
func (e *Engine) mintJIT(ctx context.Context, t Target, name string, labelList []string) (*ghapi.JITConfig, error) {
	jm, ok := e.Minter.(JitMinter)
	if !ok || t.RunnerGroup != "" {
		return nil, nil
	}
	return jm.GenerateJITConfigFor(ctx, t.Scope, t.Slug, ghapi.JITConfigRequest{
		Name:          name,
		RunnerGroupID: 1, // the Default runner group
		Labels:        labelList,
		WorkFolder:    "_work",
	})
}
...
func (e *Engine) Prepare(ctx context.Context, log io.Writer) error {
	...
	if e.RunnerVersion == "" {
		...
		if v == "" {
			...
			if err != nil {
				fmt.Fprintf(log, "homeplate: could not reach GitHub for the latest runner version (%v); using %s\n",
					err, FallbackVersion)
...
func (e *Engine) RunOne(ctx context.Context, t Target, log io.Writer) (*JobOutcome, error) {
	class := t.Class
	if class == "" {
		class = labels.ClassLinux
	}
	out := &JobOutcome{Class: class}
	...
	labelList := t.Labels
	if len(labelList) == 0 {
		labelList = labels.LabelsForClass(class)
	}

	// Per-job scratch: holds the credential file and the mounted workspace.
	...
	defer func() {
		...
		_ = shred(filepath.Join(jobDir, "jitconfig"))
		_ = DestroyWorkspace(jobDir)
	}()

	// Credential: prefer a JIT config (minted server-side, single-use, never
	// a reusable secret). Fall back to the registration-token flow when the
	// API rejects it (older GHES) or a named runner group is in play (group
	// IDs need a lookup Homeplate does not do in v1).
	var credPath, credContainerPath string
	jit, jitErr := e.mintJIT(ctx, t, runnerName, labelList)
	switch {
	case jitErr == nil && jit != nil && jit.EncodedJITConfig != "":
		credPath = filepath.Join(jobDir, "jitconfig")
		credContainerPath = "/run/homeplate/jitconfig"
		if err := os.WriteFile(credPath, []byte(jit.EncodedJITConfig), 0o600); err != nil {
			return out, err
		}
	default:
		if jitErr != nil && log != nil {
			fmt.Fprintf(log, "homeplate: JIT config unavailable (%v); using registration token\n", jitErr)
		}
		...
		credPath = filepath.Join(jobDir, "token")
		credContainerPath = "/run/homeplate/token"
		if err := os.WriteFile(credPath, []byte(tok.Token), 0o600); err != nil {
			return out, err
		}
	}
	...
	spec.Mounts = []Mount{{Host: credPath, Container: credContainerPath, ReadOnly: true}}
	host := t.Host
	if host == "" {
		host = e.Host
	}
	spec.Env = map[string]string{
		"HOMEPLATE_URL":    t.URL(host),
		"HOMEPLATE_NAME":   runnerName,
		"HOMEPLATE_LABELS": strings.Join(labelList, ","),
		"HOMEPLATE_GROUP":  t.RunnerGroup,
	}
	spec.Labels["homeplate.target"] = t.Slug
	spec.Labels["homeplate.profile"] = t.Profile
...
func RunnerName(slug string) string {
	...
	name := fmt.Sprintf("homeplate-%s-%s-%d-%s",
