...
//
// Homeplate makes two distinct promises, and it is important not to conflate them:
//
...
type ClamshellVerdict struct {
	// WillKeepRunning is the headline YES/NO shown by `homeplate status`.
...
// String renders the verdict the way `homeplate status` prints it.
...
// Report bundles everything `homeplate status` and `homeplate doctor` print.
type Report struct {
	State     State
