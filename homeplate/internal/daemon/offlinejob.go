...
func (d *Daemon) runOfflineJob(ctx context.Context, job *store.Job) (*offline.Result, error) {
	...
	if _, err := os.Stat(filepath.Join(mirror.Path, "HEAD")); err != nil {
		return nil, fmt.Errorf("no local mirror for %s. Homeplate mirrors linked repos on each "+
			"connected sync; run `homeplate link` while online first", job.RepoSlug)
	}
	...
	if !mirror.HasCommit(ctx, sha) {
		return nil, fmt.Errorf("commit %s is not in the local mirror of %s; "+
			"push it to the mirror or run `homeplate run --repo-path <dir>`", short(sha), job.RepoSlug)
	}
	...
	// secrets will fail offline, and Homeplate says so rather than pretending.
